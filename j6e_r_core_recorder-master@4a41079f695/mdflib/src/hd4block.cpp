...
size_t Hd4Block::Write(std::FILE* file) {
  ...
  if (!update) {
    ...
    // ├───────────────────────────┼───────────────┼────────────────────────┤
    // │ Base block header         │ 24            │ Includes identifiers   │
    // │ Link fields (6 total)     │ 6 × 8 = 48    │ Each link is 8 bytes   │
    // │ Timestamp                 │ 8             │ Absolute start time    │
    // │ Time class                │ 4             │ uint32_t field         │
    // │ Flags                     │ 4             │ uint32_t field         │
    // │ Single-byte fields        │ 1 + 1 + 1 + 1 │ Version, flags, etc.   │
    // │ Start angle               │ 8             │ uint64_t or double     │
    // │ Start distance            │ 8             │ uint64_t or double     │
    // ├───────────────────────────┼───────────────┼────────────────────────┤
    // │ **Total**                 │ 108           │ Total block length     │
    // └───────────────────────────┴───────────────┴────────────────────────┘
  ...
  printf("after timestamp written bytes = %d\n", bytes);
