...
void Cg4Block::WriteSample(FILE *file, uint8_t record_id_size,
  ...
  // 获取写入前的文件偏移量
  long startOffset = ftell(file);
  if (startOffset == -1L) {
      std::cerr << "Failed to get file offset before write." << std::endl;
      return;
  }
  ...
  if (endOffset == -1L) {
      std::cerr << "Failed to get file offset after write." << std::endl;
      return;
  }


  // 打印文件中写入的范围
  std::cout << "File written range: [" << startOffset
            << " - " << (endOffset - 1) << "]" << std::endl;
  IncrementSample();            // Increment internal sample counter
  NofSamples(Sample());
}
