...
void MdfWriter::WriteSamples() {
  {
    std::lock_guard<std::mutex> lock(locker_);
    write_state_ = WriteState::WriteSamples;
    new_signal_received_ = true;
  }
  sample_event_.notify_one();
}
...
void MdfWriter::StopMeasurement(ITimestamp& stop_time) {
  StopMeasurement(stop_time.GetUtcTimeNs());
}
...
void MdfWriter::WorkThread() {
  do {
    ...
    sample_event_.wait_for(lock, 10s, [&] { return new_signal_received_.load(); });
    new_signal_received_ = false;
    switch (write_state_) {
...
void MdfWriter::SaveQueue(std::unique_lock<std::mutex>& lock) {
  // Only needed for MDF4 and SaveQueue process.
}

void MdfWriter::CleanQueue(std::unique_lock<std::mutex>& lock) {
  // Only needed for MDF4 and CleanQueue storage
}
