...
size_t Dg4Block::Write(std::FILE* file) {
  ...
  if (!update) {
    printf("update DG Block\n");
    block_type_ = "##DG";
    block_length_ = 24 + (4 * 8) + 8;
    link_list_.resize(4, 0);
  }

  printf("begin to write dg4 block\n");
  ...
  // last in the file
  printf("begin to WriteLink4List\n");
  printf("block_list_ size = %d\n", block_list_.size());
  printf("mdf block = %d\n", block_list_[0]->BlockLength());
